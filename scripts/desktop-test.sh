...
mkdir -p .clickable
tmp_config="$(mktemp .clickable/nextnews-desktop-test.XXXXXX.yaml)"
desktop_env_file=".clickable/nextnews-desktop-env.local"
cleanup() {
    rm -f "$tmp_config"
...
env_vars = {
    "NEXTNEWS_DESKTOP_TEST_AUTH": "1",
    "NEXTNEWS_TEST_SERVER": os.environ["NEXTNEWS_TEST_SERVER"],
    "NEXTNEWS_TEST_USERNAME": os.environ["NEXTNEWS_TEST_USERNAME"],
    "NEXTNEWS_TEST_APP_PASSWORD": os.environ["NEXTNEWS_TEST_APP_PASSWORD"],
}
if dark_mode:
    env_vars["NEXTNEWS_DESKTOP_DARK_MODE"] = "1"
