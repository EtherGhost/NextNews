...
- Node-backed behavior tests execute extracted QML JavaScript core modules directly: `AuthCore.js`, `NewsApiCore.js`, and `SyncPlanner.js`.
...
The JavaScript core tests verify URL normalization, News API URL/payload/parse behavior, and sync queue planning without launching the full Ubuntu Touch app.

Live Nextcloud integration tests are opt-in because they create, rename, move, and delete folders/feeds on a real test account:
...
- `NEXTNEWS_RUN_LIVE_TESTS=1`
- `NEXTNEWS_TEST_SERVER`
- `NEXTNEWS_TEST_USERNAME`
- `NEXTNEWS_TEST_APP_PASSWORD`
- `NEXTNEWS_TEST_FEED_URL` (optional; defaults to `https://xkcd.com/rss.xml`)

Use a dedicated Nextcloud user and an app password. Do not use a personal account or main password. The live suite uses a `NextNewsLiveTest-` prefix and attempts cleanup before and after each test.
